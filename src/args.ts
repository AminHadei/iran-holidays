...
export function normalizeYear(input: string): number {
  const english = input
    .trim()
    .replaceAll(/[۰-۹]/gu, (digit) => String(PERSIAN_DIGITS.indexOf(digit)))
    .replaceAll(/[٠-٩]/gu, (digit) => String(ARABIC_DIGITS.indexOf(digit)));
  if (!/^\d+$/u.test(english)) return Number.NaN;
