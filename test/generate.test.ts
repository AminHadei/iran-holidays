...
test('1404 holidays match the official calendar and skip international occasions', () => {
  ...
  assert.match(arbaeen?.holidayDescription ?? '', /اربعین حسینی/u);

  for (const day of year.data) {
    if (!day.isHoliday) assert.equal(day.holidayDescription, null);
    assert.doesNotMatch(day.holidayDescription ?? '', /کریسمس|روز جهانی|سال نو میلادی/u);
  }
});
...
test('30 Esfand exists only in a leap year and is a holiday', () => {
  ...
  assert.match(lastDay?.holidayDescription ?? '', /آخرین روز سال/u);
  assert.equal(
    generateYear(1404).data.some((day) => day.date === '1404/12/30'),
    false,
  );
});
...
test('holidays start only after the event they commemorate', () => {
  ...
  assert.equal(
    year10.data.some((day) => day.holidayDescription?.includes('نیمه شعبان')),
    false,
  );
  assert.equal(
    year10.data.some((day) => day.holidayDescription?.includes('قائم')),
    false,
  );
  ...
  assert.match(
    year1340.data.find((day) => day.date === '1340/12/29')?.holidayDescription ?? '',
    /صنعت نفت/u,
  );

  const revolution = generateYear(1357);
  assert.match(
    revolution.data.find((day) => day.date === '1357/11/22')?.holidayDescription ?? '',
    /انقلاب اسلامی/u,
  );
});
